...
public class ConceptNetRetokenizer implements Function<List<String>, List<Integer>> {
    private Map<String, Integer> vocab = new HashMap<>();
    private int outOfVocabularyCount = 0;

    public static ConceptNetRetokenizer load(String path) throws IOException {
        ...
        ConceptNetRetokenizer tokenizer = new ConceptNetRetokenizer();
        String line;
        int idx = 0;
    ...
    @Override
    public void apply(Record<List<String>> input, ResultCallback<List<Integer>> callback) {
        ...
        while(!value.isEmpty()){
            ...
            if(vocab.containsKey(startToken)){
                ...
                result.add(vocab.get(longestToken));
            }else{
